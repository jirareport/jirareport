...
import br.com.jiratorio.jira.client.extension.headersWithoutAuth
import feign.Logger
...
class FeignLogger : Logger() {
    ...
    override fun logAndRebufferResponse(configKey: String, logLevel: Level, response: Response, elapsedTime: Long): Response {
        log.info("Method=logAndRebufferResponse, configKey={}, status={}, elapsedTime={}", configKey, response.status(), elapsedTime)

        if (logLevel.ordinal >= Level.HEADERS.ordinal) {
            log.info("Method=logAndRebufferResponse, configKey={}, headers={}", configKey, response.headersWithoutAuth())
            ...
            if (response.body() != null && !(response.status() == 204 || response.status() == 205)) {
                ...
                if (logLevel.ordinal >= Level.FULL.ordinal && bodyData.isNotEmpty()) {
                    log.info("Method=logAndRebufferResponse, configKey={}, body={}", configKey, Util.decodeOrDefault(bodyData, Util.UTF_8, "Binary data"))
                }
    ...
    override fun logIOException(configKey: String, logLevel: Level, ioe: IOException, elapsedTime: Long): IOException {
        log.info("Method=logIOException, configKey={}, elapsedTime={}, E={}, Err={}", configKey, elapsedTime, ioe.javaClass.simpleName, ioe.message)
